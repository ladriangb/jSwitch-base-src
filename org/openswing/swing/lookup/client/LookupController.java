...
public class LookupController {
    ...
    /** lookup value object */
    protected ValueObject lookupVO = null;
    /** lookup frame title */
    ...
    protected int selectedRow = -1;//AND1
    ...
    protected boolean codeValid = true;//AND1
    ...
    protected boolean disableFrameClosing = false;//AND1
    ...
    /** lookup frame reference 
     */
    public LookupFrame getLookupFrame() {//AND1
        return this.lookupFrame;
    }
...
//          fireCodeChangedEvent(lookupParent.getValueObject());
     ...
     */
    protected void updateParentModel(LookupParent lookupParent) {
...
//      table.reload();
     ...
     * @version 1.0
     */
    public class LookupFrame extends JDialog {//AND1
    ...
    /** Select from alias of consulted columns
     * @param separator
     * @param tableName
     * @return  
     */
    public String createSelect(String tableName, String separator) {
        String select = "";
        int length = colProperties.length;
        for (int i = 0; i < length; i++) {
            if (!colProperties[i].isVisible()) {
                continue;
            }
            String columnName = colProperties[i].getColumnName();
            if (select.isEmpty()) {
                select = "SELECT ";
            } else {
                select += ", ";
    ...
    /**
     * remueve las columnas q no se esten usando
     */
    public void removeUnvisibleColumns() {
        ArrayList<Column> co = new ArrayList<Column>(0);
        for (int i = 0; i < colProperties.length; i++) {
            if (colProperties[i].isVisible()) {
                co.add(colProperties[i]);
            }
        }
        colProperties = co.toArray(new Column[0]);
    }
